...
def main (): # 主程序
    ...
    code = eval (result.communicate () [0].decode ("utf-8")).get ("code") # 获取命令行返回值，并改变编码为“utf-8”，转换类型为“词典”，并获取API的返回值
    ...
    if "3" in choose: # 用户的选择中是否有“3”（分享）
        print ("正在完成任务：分享（5经验）……")
        subprocess.Popen ('curl -b "SESSDATA=' + SESSDATA + ';bili_jct=' + bili_jct+'" -d "aid=' + video [0] + '&csrf=' + bili_jct + '" "https://api.bilibili.com/x/web-interface/share/add"', shell = True) # “分享”视频，实际上没有分享，只是告诉B站用户已经分享了视频
